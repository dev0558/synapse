...
```
Plan  →  Retrieve (red and blue tools)  →  Web-ground  →  Synthesize  →  Verify
```
...
| Module | What it does |
|--------|--------------|
| **Analyze** | The visible agent pipeline and the red/blue cited brief, ending in a self-critique pass. |
...
| **Dashboard** | Live charts over over 1,600 real CISA KEV entries · top affected vendors, weakness-class (CWE) distribution, and additions over time. |
...
| `kev_check` | blue | CISA KEV lookup → exploited-in-the-wild status and deterministic patch priority |
...
- **Framework:** Next.js (App Router) and TypeScript, streamed responses over Node route handlers
...
- **Data:** CISA Known Exploited Vulnerabilities catalog and MITRE ATT&CK technique mappings
- **Deploy:** Vercel
...
Built with **Google Antigravity**, **Gemini API**, and **Next.js**.
